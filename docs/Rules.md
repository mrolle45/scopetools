...
<details>
<summary><h2 id="sys-req"> System Requirements </h2></summary>Most of the flags in the scope context are modeled closely, but not exactly, after the internal symbol table used by the compiler.
...
A ns is created in the following ways:
...
The result is the same as `ns.binder(var)` if `ns.context(var)` has the CELL bit set, and None otherwise.
